	...
	$doc.ready(function() {
	   ...
	   $('.wrapper').on( 'click', '.tabs-nav a', function(event) {
		...
		function open_ballon(id, dataId, el, elHref) {
			...
			var colore = "";
			var htmlcontents = $(elemHref).html();
			var sorting = 0;
		...
		function open_ballon_half(id, dataId, el, elHref) {
			...
			var newItem = 	$('<div id="ballon" data-item-id="'+ id + '" class="tile ballon ballon-half">'+htmlcontents+'<p class="sorting">'+sorting+'</p></div>');
			...
			currentGrid.append( newItem ).isotope('appended', newItem);
